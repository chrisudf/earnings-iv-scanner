...
**结论行**:当推荐=0 **且至少有一只 🟡 可考虑**时,信号列表顶部会加一句结论,例如
`⛔ 结论: 无符合策略的标的。下列 4 只全部倒在「IV 不够贵」...`。
理由是四条 🟡 可考虑 排在一起,凌晨五点扫一眼极易读成"有 4 个候选可以下单",
而正确读法是"今晚没有符合策略的标的"。

这行在两种情况下静默:有任何推荐标的时(逐条信息已经够了);以及推荐和可考虑**都为 0**
时(全场只剩 AVOID/NO_DATA)——那种场景下正文本身就是「本次扫描没有推荐或可考虑的标的」,
再加一句结论是重复。preview 班次会附加"距开仓还有数小时,以 confirm 班次为准",
因为它不是最终判断。

**预期波动可能显示为「不可用」**:跨式只有**两条**腿(前月 ATM 的看涨和看跌),但要看
四个报价——两条腿各自的买价和卖价。这四个里只要有一个缺失或为 0(常见于 ZERO_BID 标记
的票),本模块就**拒绝**用 `ask/2` 拼一个虚构的跨式价,该字段直接标为不可用。后月合约的
报价不参与这个判断。这是刻意的——虚构的 straddle 会把预期波动算高,正是 WIT/FNB
那类误判的来源。
